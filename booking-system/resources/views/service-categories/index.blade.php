                ...
                <tbody>
                    @forelse($categories as $category)
                        <tr style="border-bottom:1px solid #eee;">
                            ...
                            <td style="padding:12px;">{{ $category->is_active ? 'Active' : 'Inactive' }}</td>
                            <td style="padding:12px; text-align:right;">
                                <a href="{{ route('service-categories.edit', $category) }}" style="color:#2563eb; text-decoration:none;">
                                    Edit
                                </a>

                                <form action="{{ route('service-categories.destroy', $category) }}" method="POST" style="display:inline-block; margin-left:12px;">
                                    @csrf
                                    @method('DELETE')
